...
const CreateTokenComponent: React.FC = () => {
  ...
  const createLiquidityToken = async () => {
    try {
      const result = await contract.methods
        .createLiquidityToken(
          ...
          tokenDetails.transactionFeeYield,
          tokenDetails.transactionFeeLiquidity,
          tokenDetails.charityAddress,
          tokenDetails.charityPercent
        )
        .send({
          from: address,
        });
  ...
  const createStandardToken = async () => {
    try {
      // Call the createLiquidityToken function
      const result = await contract.methods
        .createStandardToken(
          tokenDetails.name,
          tokenDetails.symbol,
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    else if (tokenDetails.tokenType == "Standard Token") {
